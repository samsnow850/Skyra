...
const ForgotScreen = () => {
  ...
  const handleForgot = async () => {
    ...
    else setMessage('You will receive an email with the reset link.');
    setLoading(false);
  };
      ...
      <SafeAreaView style={{ flex: 1, backgroundColor: theme.backgroundGradient[0] }}>
        ...
        <KeyboardAvoidingView behavior={Platform.OS === 'ios' ? 'padding' : undefined} style={{ flex: 1, justifyContent: 'center', alignItems: 'center', padding: 24 }}>
          <View style={[styles.card, { backgroundColor: theme.card }]}> 
            ...
            <Text style={[styles.subtitle, { color: theme.text + '99' }]}>You will receive an email with the reset link.</Text>
            ...
            <TouchableOpacity style={[styles.button, { backgroundColor: theme.accent }]} onPress={handleForgot} disabled={loading}>
              <Text style={[styles.buttonText, { color: theme.card }]}>{loading ? 'Loading...' : 'Continue'}</Text>
            </TouchableOpacity>
